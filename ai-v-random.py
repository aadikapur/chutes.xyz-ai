...
def tryMovesUntilOneWorksThenMove(game,action):
  global legalMoves,bombs,parachutes,soldierSpyMoveList,tankMoveList
...
def move(game, agent, agentnum, epsilon):
  #Before Move
  global bombs, parachutes, legalMoves
  done = False
...
for episode in range(EPISODES):
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        average_reward = sum(ep_rewards1[-AGGREGATE_STATS_EVERY:])/len(ep_rewards1[-AGGREGATE_STATS_EVERY:])
        min_reward = min(ep_rewards1[-AGGREGATE_STATS_EVERY:])
        max_reward = max(ep_rewards1[-AGGREGATE_STATS_EVERY:])
        # Save model, but only when min reward is greater or equal a set value
        if min_reward >= MIN_REWARD:
            agent1.model.save('./models/{}-avgrwrd-{}.model'.format(MODEL_NAME,average_reward))
#    ep_rewards2.append(episode_reward[1])
#    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
#        average_reward = sum(ep_rewards2[-AGGREGATE_STATS_EVERY:])/len(ep_rewards2[-AGGREGATE_STATS_EVERY:])
#        min_reward = min(ep_rewards2[-AGGREGATE_STATS_EVERY:])
#        max_reward = max(ep_rewards2[-AGGREGATE_STATS_EVERY:])
#        # Save model, but only when min reward is greater or equal a set value
#        if min_reward >= MIN_REWARD:
#            agent2.model.save(f'/content/drive/My Drive/models/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.model')
...
plt.plot(np.arange(10_000),drawY)
plt.savefig('draws.png')
plt.plot(np.arange(10_000),p1winsY)
plt.savefig('p1wins.png')
plt.plot(np.arange(10_000),p2winsY)
plt.savefig('p2wins.png')
plt.plot(np.arange(10_000),numTurnsInGameY)
