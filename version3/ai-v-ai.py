...
agent1 = DQNAgent()
agent2 = DQNAgent()
agent1.model = load_model('./models/trained-against-itself.model')
agent2.model = load_model('./models/trained-against-itself.model')
ep_rewards1 = []
ep_rewards2 = []
draws = 0
p1wins = 0
...
def move(game, agent, agentnum, epsilon):
  ...
  if winner is None:
    ...
    #assign reward
    if new_other_bases<other_bases and new_bases>=current_bases:
        reward+=0.1
        if agentnum==2:
            rewardtype[0]+=1
    if new_bases>current_bases and new_other_bases<=other_bases:
        reward+=0.05
        if agentnum==2:
            rewardtype[1]+=1
    reward-=tax
...
for episode in range(EPISODES):
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        #agent1.model.save('./models/{}_p1_moves{}__g{}_e{}.model'.format(MODEL_NAME,draws+p1wins+p2wins, p1wins,epsilon1))
        #win_percent2 = sum(ep_rewards2[-AGGREGATE_STATS_EVERY:])/AGGREGATE_STATS_EVERY
        # Save model, but only when min reward is greater or equal a set value
        agent2.model.save('./models/model-g{}.model'.format(episode))
        #if win_percent1<0.1:
        #    epsilon1/=0.99
        #elif win_percent2<0.1:
        #    epsilon2/=0.99
        #if episode>1:
        #    agent1.model = load_model('./models/model-g{}.model'.format(episode))
